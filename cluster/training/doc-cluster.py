...
def populate_job_cities_table(conn, cities, df):
    ...
    for index, row in df.iterrows():
        ...
        for city in temp:
            ...
            query = "INSERT INTO JOB_CITIES(job_id, city_id) VALUES({}, {})".format(
                job_id, city_id
            )
            c.execute(query)
            count = count + 1
...
def finalizeCompact(extra_info, clusters_kmeans_tfidf, clusters_kmeans_cnt, dataset):
    ...
    data_df = data_df[
        data_df[data_df.columns[2]].apply(lambda x: False if x in diff else True)
    ]
    ...
    data = {
        "id": extra_info[0],
        "exp": extra_info[1],
        "max_exp": extra_info[2],
        "position": extra_info[3],
        "ed_status": extra_info[5],
        "text": text,
        "text_clear": text_cleared,
    }
  ...
  """
    ...
    data1 = {
        "job_id": extra_info[0],
        "clusters_tfidf": clusters_kmeans_tfidf,
        "clusters_cnt": clusters_kmeans_cnt,
    }
  ...
  """
    ...
    cities_df = pd.DataFrame(
        {"id": np.arange(len(cities_unique)), "name": cities_unique}
    )

    create_job_cities_table(conn, cities_df)
    populate_job_cities_table(
        conn,
        cities_df,
        pd.DataFrame({"job_id": extra_info[0], "cities": extra_info[4]}),
    )
...
if __name__ == "__main__":
    # read file
    df = pd.read_csv(
        "../../data/ie-result.lm.txt", encoding="utf-8", header=None, sep=";"
    )
    ...
    clustering(
        wordsLemmatized, stopwords, extra_info, dataset="../../data/dataset.xlsx"
    )